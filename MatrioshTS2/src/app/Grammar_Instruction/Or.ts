...
class Or extends Expresion
{
    constructor(p_fila : number, p_columna: number, p_operador_izq : Instruction, p_operador_der : Instruction) {
        super(p_fila,p_columna,tipo_operacion.OR,p_operador_izq, p_operador_der);
    }
    ...
    public analizar(entorno_padre : Entorno, nivel : number)
    {
        ...
        try
        {
            ...
            if (!(op1.getTipo().getTipo() == tipo_dato.BOOLEANO) || !(op2.getTipo().getTipo() == tipo_dato.BOOLEANO))
            {
                ...
                _return.setFila(this.fila);
                _return.setColumna(this.columna);
                _return.setMensaje("No es posible realizar Or con valores no booleanos.");
                return _return;
            }
            else
            {
    ...
    public getThis() 
    {
        return new Or(this.fila,this.columna,this.operador_izq.getThis(),this.operador_der.getThis());
    }
}
